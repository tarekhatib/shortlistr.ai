...
RF_MODEL_PATH  = os.path.join(os.path.dirname(__file__), "rf_model.joblib")
...
def load_models() -> tuple[Any, Any, StandardScaler, pd.Series]:
    rf_data = joblib.load(RF_MODEL_PATH)
    mlp     = keras.models.load_model(MLP_MODEL_PATH)
    scaler  = joblib.load(SCALER_PATH)
    return rf_data["rf"], mlp, scaler, rf_data["benchmarks"]
...
def predict(pdf_path: str, jd_path: str) -> None:
    ...
    if rf_pred == 0:
        ...
        print("=" * 55)
        for item in generate_feedback(features_enc, rf, benchmarks, {}):
            print(f"  • {item}")

    print("\n" + "=" * 55)
    print("  DL Model — Shallow Neural Network (5→6→1)")
    print("=" * 55)
